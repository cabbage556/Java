...
public class StringSample {
    ...
    public void convert() {
        try {
            // "감바스" 값을 갖는 String 객체 생성
            String korean = "감바스";

            // getBytes()
            //      String 객체의 문자열 값을 byte 배열로 변환함
            //      기본 캐릭터셋 사용
            byte[] arr1 = korean.getBytes();
            printByteArray(arr1);

            // String(byte[])
            //      byte 배열로 String 객체 생성
            //      기본 캐릭터셋 사용
    ...
    public void convertUTF16() {
        // try-catch 블록 사용 이유
        //      UnsupportedEncodingException 예외 처리를 위해
        //      getBytes(String charsetName) 메서드와 String(byte[] bytes, String charsetName) 생성자에
        //      전달하는 캐릭터셋이 존재하지 않는 캐릭터셋일 경우 이 예외가 발생함
        try {
            String korean = "감바스";

            // getBytes(String charsetName)
            //      String 객체를 byte 배열로 변환함
